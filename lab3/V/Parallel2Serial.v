module Parallel2Serial( parallelDataIn, syncClock, serialDataOut, syncOut );
// Port Declaration
input wire [ 11: 0 ] parallelDataIn; // 12 bit sample
input wire syncClock; // Serialization clock is generated by PLL and fed into module

output wire serialDataOut; // Output data to DAC
...
always @( posedge syncClock ) begin

    cycles = cycles + 1'b1;

    if ( cycles == 8'd160 ) begin
        // resets the counter after 160 cycles
        cycles = 1'b0;
    end
    ...
    if ( cycles == 1'b0 ) begin
        ...
        content <= { 4'b0, COMMANDBITS, ADDRESSBITS, parallelDataIn, 8'b0 };
    end
...
assign serialDataOut = content[ 31 ];
assign syncOut = sync;
